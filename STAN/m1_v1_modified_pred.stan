functions {
  ...
  // find rank with highest choice probability in theta
  int ranking(vector theta, int J) {
    
    real highest_prob;
    highest_prob = max(theta);
    
    int index;

    for (j in 1:J) {
      
      if(theta[j] == highest_prob) {
        index = j;
      }
...
generated quantities {
  ...
  for (t in 1:T) {
    ...
    for (n in 1:N) {
      ...
      // qualifier/race ranking equation
      // model always predicts rank with highest choice probability
      R_pred[n,t] = ranking(theta, J);
