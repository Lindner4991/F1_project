  ...
  for (t in 1:T) {
    ...
    if (t == 1) {
      ...
      for (k in 1:K) {
        ...
        if (I_3[k,t] == 1) { mu_C[k,t] = normal_rng(mu_C_0[k], varsigma_C + kappa_C * i_5[t]); }
    ...
    if (t > 1) {
      ...
      for (k in 1:K) {
        ...
        if (I_3[k,t] == 1) { mu_C[k,t] = normal_rng(mu_C[k,t-1], varsigma_C + kappa_C * i_5[t]); }
    ...
    for (n in 1:N) {
      ...
      real sigma_P;
      sigma_P = sqrt(square(varsigma_D + kappa_D * I_4[n,t]) + square(varsigma_C + kappa_C * i_5[t]));
